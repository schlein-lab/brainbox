...
DEVICE = os.environ.get("PP_WHISPER_DEVICE", "cpu")
COMPUTE = os.environ.get("PP_WHISPER_COMPUTE", "int8")
...
wm = WhisperModel(WHISPER, device=DEVICE, compute_type=COMPUTE)
...
_CATALOG_CACHE = {}
_WHISPER_FALLBACK = ["tiny", "base", "small", "medium", "large-v3"]
_LANG_FALLBACK = [
    {"code": "de", "name": "Deutsch", "voices": [{"key": "de_DE-thorsten-medium", "quality": "medium"}]},
    {"code": "en", "name": "English", "voices": [{"key": "en_US-lessac-medium", "quality": "medium"}]},
]
_WHISPER_RAM = {"tiny": 500, "base": 700, "small": 1400, "medium": 3200,
                "large-v1": 5200, "large-v2": 5200, "large-v3": 5200,
                "large-v3-turbo": 2600, "turbo": 2600, "distil-large-v2": 2600,
                "distil-large-v3": 2600, "distil-medium.en": 1400, "distil-small.en": 700}

def _whisper_ram(name):
    return _WHISPER_RAM.get(name, 3000)

def _http_json(url, timeout=8):
    req = urllib.request.Request(url, headers={"User-Agent": "brainbox-voice/1"})
...
def voice_hardware():
    ...
    if not gpu and os.path.exists("/dev/nvidia0"):
        gpu = True
    return {"ram_total_mb": ram_total, "ram_avail_mb": ram_avail, "cores": cores,
            "gpu": gpu, "gpu_name": gpu_name}

def _fetch_whisper_models(timeout=8):
    try:
        raw = _http_json("https://huggingface.co/api/models?author=Systran&search=faster-whisper&limit=80", timeout)
    except Exception:
        return None
    names = []
    for m in raw:
        mid = m.get("id", "") or m.get("modelId", "")
        if "faster-distil-whisper-" in mid:
            names.append("distil-" + mid.split("faster-distil-whisper-")[-1])
        elif "faster-whisper-" in mid:
            names.append(mid.split("faster-whisper-")[-1])
    ...
    for n in names:
        n = n.strip()
        if n and n not in seen and ".en" not in n or (n in order):
            if n and n not in seen:
                seen.append(n)
    seen.sort(key=lambda n: order.index(n) if n in order else 999)
    return seen or None

def _recommend_model(hw, models):
    avail = hw.get("ram_avail_mb") or hw.get("ram_total_mb") or 2000
    if hw.get("gpu"):
        prefs = ["large-v3", "medium", "small", "base", "tiny"]
    elif avail >= 9000:
        prefs = ["medium", "small", "base", "tiny"]
    elif avail >= 5000:
        prefs = ["small", "base", "tiny"]
    elif avail >= 2500:
        prefs = ["base", "small", "tiny"]
    else:
        prefs = ["tiny", "base"]
    for p in prefs:
        if p in models:
            return p
    return models[0] if models else "small"

def _fetch_voices_json(timeout=8):
    now = time.time()
    if _CATALOG_CACHE.get("voices") and now - _CATALOG_CACHE.get("voices_t", 0) < 3600:
        return _CATALOG_CACHE["voices"]
    try:
        vj = _http_json("https://huggingface.co/rhasspy/piper-voices/resolve/main/voices.json", timeout)
        _CATALOG_CACHE["voices"] = vj
        _CATALOG_CACHE["voices_t"] = now
        return vj
    except Exception:
        return _CATALOG_CACHE.get("voices")

def _fetch_piper_languages(timeout=8):
    vj = _fetch_voices_json(timeout)
    if not vj:
        return None
    langs = {}
    for key, e in vj.items():
        lang = e.get("language") or {}
        code = lang.get("code") or lang.get("family") or ""
        if not code:
            continue
        name = lang.get("name_native") or lang.get("name_english") or code
        q = e.get("quality") or ""
        langs.setdefault(code, {"code": code, "name": name, "voices": []})
        langs[code]["voices"].append({"key": key, "quality": q})
    return sorted(langs.values(), key=lambda x: x["name"])

def _default_voice_for_lang(lang):
    vj = _fetch_voices_json()
    if vj:
        cands = []
        for key, e in vj.items():
            code = (e.get("language") or {}).get("code") or ""
            if code == lang or code.split("_")[0] == (lang or "").split("_")[0]:
                cands.append((e.get("quality") or "", key))
        for want in ("medium", "high", "low", "x_low", ""):
            for q, key in cands:
                if q == want:
                    return key
        if cands:
            return cands[0][1]
    return "de_DE-thorsten-medium" if (lang or "").startswith("de") else "en_US-lessac-medium"

def _download_piper_thorsten():
    base = "https://huggingface.co/rhasspy/piper-voices/resolve/main/de/de_DE/thorsten/medium/"
    onnx = ""
    for suf in ("de_DE-thorsten-medium.onnx", "de_DE-thorsten-medium.onnx.json"):
        dest = os.path.join(PIPER_DIR, suf)
        if not (os.path.exists(dest) and os.path.getsize(dest) > 1000):
            try:
                ...
                print("  Download-Fehler:", str(e)[:90])
        if suf.endswith(".onnx"):
            onnx = dest
    return onnx

def _download_piper_voice(voice_key):
    if not voice_key:
        return ""
    vj = _fetch_voices_json()
    entry = (vj or {}).get(voice_key)
    if not entry:
        return _download_piper_thorsten()
    ...
    for rel in (entry.get("files") or {}):
        ...
        if os.path.exists(dest) and os.path.getsize(dest) > 1000:
            if rel.endswith(".onnx"):
                onnx_path = dest
            continue
        print("lade Piper-Stimme:", os.path.basename(rel))
        try:
            urllib.request.urlretrieve(base + rel + "?download=true", dest)
            if rel.endswith(".onnx"):
                onnx_path = dest
        except Exception as e:
            print("  Download-Fehler:", str(e)[:90])
    return onnx_path or _download_piper_thorsten()

def voice_catalog(timeout=8):
    hw = voice_hardware()
    models = _fetch_whisper_models(timeout)
    live = bool(models)
    models = models or list(_WHISPER_FALLBACK)
    rec = _recommend_model(hw, models)
    ml = [{"name": m, "approx_ram_mb": _whisper_ram(m), "recommended": (m == rec)} for m in models]
    langs = _fetch_piper_languages(timeout)
    live = live and bool(langs)
    langs = langs or _LANG_FALLBACK
    return {"ok": True, "hardware": hw, "models": ml, "recommended": rec,
            "languages": langs, "live": live}
...
def _voice_runtime():
    try:
        ...
        cfg = {}
    model = cfg.get("voice_model") or WHISPER_SIZE or "small"
    device = cfg.get("voice_device") or "cpu"
    compute = "float16" if device == "cuda" else "int8"
    piper = cfg.get("voice_piper_model") or PIPER_MODEL or ""
    lang = cfg.get("voice_lang") or "de"
    return model, device, compute, piper, lang

def voice_install_pkg(model="small", lang="de", voice_key="", device="cpu"):
    ...
    piper_model = _download_piper_voice(voice_key or _default_voice_for_lang(lang))
    compute = "float16" if device == "cuda" else "int8"
    print("lade Whisper-Modell (%s, %s/%s) ..." % (model, device, compute))
    subprocess.run([VENV_PY, "-c",
        "from faster_whisper import WhisperModel; WhisperModel(%r, device=%r, compute_type=%r)"
        % (model, device, compute)], check=False)
    open(VOICED_PY_PATH, "w").write(VOICED_PY)
    cfg = load_cfg()
    caps = cfg.setdefault("caps", {})
    caps["voice"] = True
    caps["voice_stt_server"] = True
    cfg["voice_model"] = model
    cfg["voice_lang"] = lang
    cfg["voice_device"] = device
    if piper_model:
        cfg["voice_piper_model"] = piper_model
    save_cfg(cfg)
    print("Voice installiert (Whisper %s / Sprache %s). Server neu starten: brainbox-portal serve" % (model, lang))

def _install_voice_cli(argv):
    model = os.environ.get("PP_WHISPER") or "small"
    lang = os.environ.get("PP_VOICE_LANG") or "de"
    voice = os.environ.get("PP_VOICE_KEY") or ""
    device = os.environ.get("PP_WHISPER_DEVICE") or "cpu"
    i = 0
    while i < len(argv):
        a = argv[i]
        ...
        if a == "--device" and i + 1 < len(argv):
            device = argv[i + 1]; i += 2; continue
        i += 1
    try:
        voice_install_pkg(model=model, lang=lang, voice_key=voice, device=device)
    except Exception as e:
        print("install-voice FEHLER:", str(e)[:300])

_VOICE_INSTALL_STATE = os.path.join(DATA_DIR, "voice-install.json")
_VOICE_INSTALL_LOG = os.path.join(DATA_DIR, "voice-install.log")

def voice_installed():
    return voice_stack_missing() is None

def _vi_load():
    try:
        with open(_VOICE_INSTALL_STATE, encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {"state": "idle"}
...
def _vi_save(d):
    os.makedirs(DATA_DIR, exist_ok=True)
    tmp = _VOICE_INSTALL_STATE + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(d, f)
    os.replace(tmp, _VOICE_INSTALL_STATE)
...
def voice_install_start(opts=None):
    opts = opts or {}
    if voice_installed() and not opts.get("force"):
        _vi_save({"state": "done"})
        return {"ok": True, "installed": True, "state": "done"}
    st = _vi_load()
    if st.get("state") == "running" and st.get("pid") and _vi_alive(st["pid"]):
        return {"ok": True, "state": "running", "started": False}
    os.makedirs(DATA_DIR, exist_ok=True)
    bp = os.path.join(os.path.dirname(os.path.abspath(__file__)), "brainbox-portal")
    env = dict(os.environ)
    if opts.get("model"): env["PP_WHISPER"] = str(opts["model"])
    if opts.get("lang"): env["PP_VOICE_LANG"] = str(opts["lang"])
    if opts.get("voice"): env["PP_VOICE_KEY"] = str(opts["voice"])
    if opts.get("device"): env["PP_WHISPER_DEVICE"] = str(opts["device"])
    try:
        log = open(_VOICE_INSTALL_LOG, "w", encoding="utf-8")
        p = subprocess.Popen([sys.executable, bp, "install-voice"], stdout=log,
                             stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL,
                             start_new_session=True, env=env)
    except Exception as e:
        _vi_save({"state": "error", "error": str(e)[:200]})
        return {"ok": False, "error": str(e)[:200]}
    _vi_save({"state": "running", "pid": p.pid, "started": int(time.time()),
              "model": opts.get("model"), "lang": opts.get("lang")})
    return {"ok": True, "state": "running", "started": True}

def voice_install_status(tail=6000):
    installed = voice_installed()
    st = _vi_load()
    state = st.get("state", "idle")
    if state == "running" and st.get("pid") and not _vi_alive(st["pid"]):
        state = "done" if installed else "error"
        st["state"] = state; _vi_save(st)
    if installed and state != "running":
        state = "done"
    log = ""
    try:
        with open(_VOICE_INSTALL_LOG, encoding="utf-8", errors="replace") as f:
            log = f.read()[-tail:]
    except Exception:
        pass
    return {"ok": True, "installed": installed, "state": state, "log": log}
...
def start_voiced():
    ...
    _vm, _vdev, _vcomp, _vpiper, _vlang = _voice_runtime()
    envv = ["/usr/bin/env",
            "PP_WHISPER=%s" % _vm,
            "PP_WHISPER_DEVICE=%s" % _vdev,
            "PP_WHISPER_COMPUTE=%s" % _vcomp,
            "PP_VOICE_LANG=%s" % _vlang,
            "PP_PIPER_BIN=%s" % (PIPER_BIN if PIPER_BIN and os.path.exists(PIPER_BIN) else ""),
            "PP_PIPER_MODEL=%s" % (_vpiper if _vpiper and os.path.exists(_vpiper) else ""),
    ...
    env = os.environ.copy()
    _vm2, _vdev2, _vcomp2, _vpiper2, _vlang2 = _voice_runtime()
    env["PP_WHISPER"] = _vm2
    env["PP_WHISPER_DEVICE"] = _vdev2
    env["PP_WHISPER_COMPUTE"] = _vcomp2
    env["PP_VOICE_LANG"] = _vlang2
    env["PP_PIPER_BIN"] = PIPER_BIN if PIPER_BIN and os.path.exists(PIPER_BIN) else ""
    env["PP_PIPER_MODEL"] = _vpiper2 if _vpiper2 and os.path.exists(_vpiper2) else ""
