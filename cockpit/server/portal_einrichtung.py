...
def dispatch(h, method, path):
    ...
    if method == "GET" and path == "/api/einrichtung":
        ...
        return True
    if path in ("/api/einrichtung/voice-katalog", "/api/einrichtung/voice-status") or \
            (method == "POST" and path == "/api/einrichtung/voice-install"):
        import portal_voice_core as _pvc
        if path == "/api/einrichtung/voice-katalog":
            h._sess_json({"ok": True, "katalog": _pvc.voice_catalog()})
            return True
        if path == "/api/einrichtung/voice-status":
            h._sess_json(_pvc.voice_install_status())
            return True
        _vb = _leib(h)
        h._sess_json(_pvc.voice_install_start({"model": _vb.get("model"), "lang": _vb.get("lang"), "voice": _vb.get("voice"), "device": _vb.get("device")}))
        return True
